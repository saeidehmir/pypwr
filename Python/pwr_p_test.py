...
def pwr_p_test(alternative, h=None, n=None, sig_level=0.05, power=None):
    ...
    if tside==1 and (h is not None):
        h = np.abs(h)
    if tside == 1:
        p_body = "norm.sf(norm.isf(sig_level/2)-h*np.sqrt(n)) + norm.cdf(norm.ppf(sig_level/2)-h*np.sqrt(n))"
    elif tside == 2:
        p_body = "norm.sf(norm.isf(sig_level)-h*np.sqrt(n))"
    elif tside == 0:
        p_body = "norm.cdf(norm.ppf(sig_level)-h*np.sqrt(n))"
    if power is None:
        power = eval(p_body)
    elif h is None:
        loc = {}
